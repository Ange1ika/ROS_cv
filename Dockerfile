...
# Установка ROS 2 (Foxy) и ROS 1 Bridge
...
# Установка зависимостей для ROS2
RUN apt-get update && \
    apt-get install -y \
...
#RUN echo "source /sources/catkin_ws/devel/setup.bash" >> ~/.bashrc
#RUN echo "source /sources/ros2_ws/install/setup.bash" >> ~/.bashrc
