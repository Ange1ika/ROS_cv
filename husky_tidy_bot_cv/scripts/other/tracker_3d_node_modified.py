...
class Tracker3D_node(Tracker3D):
    def __init__(self, objects_topic, depth_info_topic, depth_topic,
        ...
        # создаём список для хранения точек
        self.point_clouds = {}
        ...
        with self.total_tm:
            ...
            with self.mutex:
                with self.track_3d_tm:
                    ...
                    # for dictionary
                    current_time = depth_msg.header.stamp.to_sec()
                    self.point_clouds[current_time] = []
                    for tracked_object_3d in self.tracked_objects:
                        position = tracked_object_3d.pose[:3]  # Координаты
                        tracking_id = tracked_object_3d.tracking_id  # tracking_id
                        self.point_clouds[current_time].append((tracking_id, position))
    ...
    def save_point_clouds(self, file_path):
        np.save(file_path, np.array(self.point_clouds))
...
if __name__ == '__main__':
    ...
    print("Spinning...")
    rospy.spin()
    point_cloud_file = '/resources/data/point_cloud.npy'
    tracker_3d_node.save_point_clouds(point_cloud_file)
    print(f"Облако точек сохранено в {point_cloud_file}")
